...
/**
 * useReducer : useState보다 더 다양한 컴포넌트 상황에 따라 다양한 형태를 다른 값으로 업데이트 해주고 싶을 경우 사용
 * 
 * const [state, dispatch] = useReducer(reducer, initState)
 * Component Event -> dispatcher(action) -> reducer(state, action) -> state
 * 
 * reducer  : 인자로 state, action 받아서 업데이트 된 state값 return
 * dispatch : reducer에 state 업데이트 요청
 * action   : reducer에서 작업할 내용 전달
 */

// reducer 정의
function reducer(state, action) {
    // action의 type에 따라 state를 다르게 처리
    switch (action.type) {
        case 'INCREMENT': 
